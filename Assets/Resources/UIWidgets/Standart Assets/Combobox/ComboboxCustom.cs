			...
			where TComponent : ListViewItem
	{
		...
		protected void UpdateComponentsCount()
		{
			...
			if (components.Count < currentIndices.Count)
			{
				...
				Enumerable.Range(0, currentIndices.Count - components.Count).ForEach(AddComponent);
			}
			else
			{
				var to_cache = components.GetRange(currentIndices.Count, components.Count - currentIndices.Count).OrderByDescending<TComponent,int>(GetComponentIndex);
